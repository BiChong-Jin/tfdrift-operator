...
```sh
make docker-build docker-push IMG=<some-registry>/tfdrift-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/tfdrift-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/tfdrift-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/tfdrift-operator/<tag or branch>/dist/install.yaml
```
